type TestimonialProps = {
  text: string;
  author: string;
};
export default function Testimonial(props: TestimonialProps) {
  return (
    <div className="bg-gray-50 dark:bg-slate-800 rounded-lg p-6 flex flex-col h-full">
      <div className="flex items-start flex-grow">
        <svg
          className="w-8 h-8 text-indigo-600 mr-3 flex-shrink-0"
          fill="currentColor"
          viewBox="0 0 24 24"
        >
          <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
        </svg>
        <p className="text-lg italic text-gray-700 dark:text-slate-300">
          &ldquo;{props.text}&rdquo;
        </p>
      </div>
      <p className="text-right font-semibold text-gray-900 dark:text-white mt-auto">
        — {props.author}
      </p>
    </div>
  );
}
